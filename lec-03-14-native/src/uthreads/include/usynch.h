...
//
//
// an auto reset_event on thread wakeup

typedef event_t auto_reset_event_t;
...
INLINE bool auto_event_value(auto_reset_event_t * event) {
	return event->signaled; 
}
...
void iq_init(intqueue_t *q);

int iq_get(intqueue_t *q);

void iq_put(intqueue_t *q, int val);
